...
$(function(){
	...
	if($gnb.size() != 0){
		...
		var $gnbDepth1 = $(".gnb .menu h2");
		var idx1 = gnbDepth1-1
		var idx2 = gnbDepth2-1
		var speed = 400;
		
		//reset
		$gnb_wrap.find("a:last").focusout(function(){$gnb_wrap.animate({"height":"30px"},speed);
		//$gnb_wrap.css("border-bottom","1px solid #3da415");
			
		});
		
		$(".right_inner").find("a:last").focusin(function(){
			$gnb_wrap.animate({"height":"30px"},speed);
			//$gnb_wrap.css("border-bottom","1px solid #3da415");
		...
		$gnb.find("a").bind("mouseenter focusin", function(){
		...
		$gnb.find("h2>a").bind("mouseenter focusin", function(){
			//$gnb_wrap.css("border","none");
		    $gnb_wrap.animate({ "height": "290px" }, speed);			
		...
		function gnbDepth() { // lnb depth
			if(idx1 != -1){
				$gnbDepth1.eq(idx1).find(">a>img").attr("src", $gnbDepth1.eq(idx1).find(">a>img").attr("src" ).replace("_off" ,"_on"));
			}
			if(idx2 != -1){
				$gnbDepth1.eq(idx1).next("ul").find("li").eq(idx2).find(">a>img").attr("src", $gnbDepth1.eq(idx1).next("ul").find("li").eq(idx2).find(">a>img").attr("src" ).replace("_off" ,"_on"));
			}
		}
		gnbDepth();
	}
...
function quick() {
    var obj = $('#quickbox.subq');
    var s_movement = function () {
        var sTop = $(window).scrollTop();
        var _path = $(location).attr('pathname');
        var _top = 0;

        //        if (_path.indexOf("/lounge/ssingLounge.aspx") > -1)
        //            _top = 256;
        if (_path.indexOf("/main/main.aspx") < 0) {
            if (sTop >= 100) {
                obj.stop().animate({ 'top': sTop }, 1000, 'easeOutExpo');
            } else {
                obj.stop().animate({ 'top': _top }, 1000, 'easeOutExpo');
            }
        }
    };
    $(window).scroll(function () {
...
// juqery-ui : selectmenu, checkbox 스크립트
$(function () {
	$(".select_basic select").each(function () {
		$(this).selectmenu()
		.selectmenu( "menuWidget" )
		.addClass( "select_overflow" );
	});

	$(".checkbox_basic input").checkboxradio();
});
/* e : 180903 */
