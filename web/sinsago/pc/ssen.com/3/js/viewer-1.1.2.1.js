...
var viewer_image_url = "../_god/html/images/viewer/";
...
PhotoMotionBox=function(frame) {
	...
	this.iMotionStyle=0;
	this.iTopMotion=this.oframe.document.body.scrollTop; //+50
	this.iWidthMotion=0;
...
PhotoMotionBox.prototype={
	Initialized : function(idObj) 
	{
		if(!this.oCreateId) {
			...
			this.setFrameCssApply({
					...
					top : this.oframe.document.body.scrollTop, //+50
					width : "300px",
					height : "300px",
					zIndex : "9999999"
				}
	...
	{
		this.Initialized();
		this.iTop=this.oframe.document.documentElement.scrollTop ? this.oframe.document.documentElement.scrollTop : this.oframe.document.body.scrollTop; //+50
		this.iWidth=width;
		this.iHeight=height;
	...
	doFrameMain : function(url,no,total) {
		...
		if(this.bFrameMainLock==false) {
			...
			this.oMotionBox.onMotionBoxFinished=function() {
				if(this.bCreated==false) {		
					// Frame Image
					if(this.oLoadImage.width >= 900) width = "width='900'";
					else width = "";

					this.oFramePhotoView.oCreateId.innerHTML="<center><img src='"+this.sImageUrl+"' " + width + " style='display:none'></center>";
			...
			this.oLoadImage.onload=function () {
				if(this.bLoadImageOnLoadBug==false) {
					...
					//2008-05-16 하근호 수정
					if(this.oLoadImage.width > 900) {
						this.oLoadImage.height= 900 * this.oLoadImage.height / this.oLoadImage.width;
						this.oLoadImage.width= 900;
					}
...
PhotoLayer.prototype={
	...
	doPhotoClick : function(e,id) {
		...
		this.n=0;
		if(id) {
			if(typeof(id) == "object") this.oImg=id.getElementsByTagName("IMG");
			else this.oImg=document.getElementById(id).getElementsByTagName("IMG");
			for(this.i=0;this.i<this.oImg.length;this.i++) {
				if(this.oImg[this.i].getAttribute("src")) {
					this.aPhotoLayer[this.j]=this.oImg[this.i].getAttribute("src");
					if(this.oImg[this.i]==e) { this.n=this.j; flag = true; }
					//if(this.oImg[this.i].src==e.getElementsByTagName("img")[0].src) 	this.n=this.j;
					this.j++;
				}
				if(flag == false) {
					this.aPhotoLayer[0] = e.src;
					this.n = 0;
				}
			}
		} else {
			if(typeof(e) == "object") this.aPhotoLayer[0] = e.src;
			else this.aPhotoLayer[0] = e;
		}
		this.doPhotoViewer();
	},
