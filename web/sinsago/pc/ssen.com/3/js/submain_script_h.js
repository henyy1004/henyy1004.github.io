$(function(){
	...
	$('.lecture .bookList .thumb a').on('mouseover',function(){
		var sbj = $(this).parent().prev('.sbj').text();
		var path = $(this).find('img').attr('src');
		$(this).parent().parent().addClass('on');
		$(this).parent().parent().siblings().removeClass('on');
		$(this).parent().parent().parent().parent().parent().find('.bookView .bookSjb span').html(sbj);
		$(this).parent().parent().parent().parent().parent().find('.bookView .thumb img').attr({src: path});
		return false;
	});
...
function bxslider(){	
	...
	$('.lecture .bx-pager .bx-pager-item').each(function(idx){
		if(idx == "0")
...
function tchChk(){
	$('.teacherRoll').children('li').removeClass('on');
	$('.teacherRoll').children('li').eq(tch1).addClass('on');
}
